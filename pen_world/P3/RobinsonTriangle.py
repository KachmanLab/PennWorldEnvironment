...
class RobinsonTriangle:
    ...
    def rhombus(self):
        """
        Return the coordinates of the rhombus (used to generate graph instead of svg)

        """

        AB, BC = self.B - self.A, self.C - self.B
        D = self.C - AB
        return np.round([self.A, self.B, self.C, D], 7)  # round so that identical vertices can be matched later
